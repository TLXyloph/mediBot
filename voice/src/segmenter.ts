// Turns the ears session's incremental transcription chunks into utterances.
// Flush on: server `finished` flag, turnComplete boundary, speaker change, or idle gap.
...
export class TranscriptSegmenter {
  ...
  /** External segment boundary (e.g. turnComplete from the Live session). */
  boundary(): void {
    this.flush();
  }
